#' Plot keywords after enrichment test as bargraph
...
#' \donttest{
#'
#' plot_kwd_bars(x = kwd_enrichment_multi_output,
#'                group_vec = categories_vector)
#' }
#' @export


plot_kwd_bars <- function(x,
                           group_vec,
                           save_output = FALSE,
                           x_discrete = FALSE){
  ...
  p <- ggplot(output, aes(keyword, ratio_test_group, fill = group, group = group)) +
